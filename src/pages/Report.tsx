...
export default function Report(){
  ...
  React.useEffect(()=>{
    (async()=>{
      setKpi(await loadCSV('/src/data/kpi_daily.csv') as any[])
      setLedger(await loadCSV('/src/data/ledger.csv') as any[])
    })()
  },[])
  ...
  const line2 = rr>0.03? '반품률이 3%를 초과. PDP 상단에 클레임 Top3 노출 권고.' : '반품률 안정 범위.'
  const line3 = edgeShare>0.30? '엣지 비중 30% 초과. 다음 월 리밸런싱 요망.' : '바벨 비중 정상 범위.'
  ...
  return (<div className='container report'>
    <div className='card'>
      <b>주간 리포트 스냅샷</b>
      <div className='hint'>프린트 버튼 → PDF로 저장 (브라우저 내장)</div>
    ...
    <div className='badges'>
      <button className='badge' onClick={doPrint}>프린트 → PDF 저장</button>
    </div>
  </div>)
}
